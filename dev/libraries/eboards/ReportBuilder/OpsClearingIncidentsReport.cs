...
{
	public class OpsClearingIncidentsReport
	{
		...
		public OpsClearingIncidentsReport()
		{
		...
		protected void FillTable(XmlDocument xdoc, XmlNode boardtable, int _round)
		{
			...
			if(File.Exists(logFile))
			{
				...
				biLogReader.WatchCreatedNodes("cleared_incidents_notifications", this.biLogReader_ClearedIncidents);
...
/*
		...
		void biLogReader_CostedEvents(object sender, string key, string line, double time)
		{
			...
			//System.Diagnostics.Debug.WriteLine(" CR " + " time  " + time.ToString() + "  Lastknowntime " + lastKnownTimeInGame.ToString());
		...
		void biLogReader_FixItQueue(object sender, string key, string line, double time)
		{
			...
			//System.Diagnostics.Debug.WriteLine(" FXQueue " + " time  " + time.ToString() + "  Lastknowntime " + lastKnownTimeInGame.ToString());
/*
			string incident_id = BasicIncidentLogReader.ExtractValue(line, "incident_id");
			if(!this.incidentToData.ContainsKey(incident_id))
			{
				// Error, shouldn't happen! (don't know what to do!).
				return;
			}

			string type = BasicIncidentLogReader.ExtractValue(line, "type");

			// Only do this if it is an "fix" or "fix by consultancy" not a workaround!
			if("fix_by_consultancy" != type)
			{
				if("fix" != type)
				{
					return;
		...
		void biLogReader_apply(object sender, string key, string line, double time)
		{
			...
			//System.Diagnostics.Debug.WriteLine(" Apply " + " time  " + time.ToString() + "  Lastknowntime " + lastKnownTimeInGame.ToString());
		...
		void biLogReader_ClearedIncidents(object sender, string key, string line, double time)
		{
			if (time > lastKnownTimeInGame) lastKnownTimeInGame = (int)time;

			//System.Diagnostics.Debug.WriteLine(" Clear " + " time  " + time.ToString() + "  Lastknowntime " + lastKnownTimeInGame.ToString());

			string incident_id_str = BasicIncidentLogReader.ExtractValue(line, "incident_id");
			//need to turn back into normal source incident number 

			int Incident_id_value = CONVERT.ParseIntSafe(incident_id_str,-1);

			//if (Incident_id_value != -1)
			//{
			//  if (Incident_id_value>100)
			//  {
			//    Incident_id_value = Incident_id_value % 100;
			//    incident_id_str = CONVERT.ToStr(Incident_id_value);
			//  }
			//}

			if (!this.incidentToData.ContainsKey(incident_id_str))
			{
				return;
			}

			FixIncident(incident_id_str, time);

			//IncidentData idata = (IncidentData)incidentToData[incident_id_str];
			//string i_name = BasicIncidentLogReader.ExtractValue(line, "i_name");
			//if (i_name.StartsWith(biz_type))
			//{
			//  string[] parts = i_name.Split(space);
		...
		void biLogReader_enteredIncidents(object sender, string key, string line, double time)
		{
			...
			//System.Diagnostics.Debug.WriteLine(" Enter " + " time  " + time.ToString() + "  Lastknowntime " + lastKnownTimeInGame.ToString());
