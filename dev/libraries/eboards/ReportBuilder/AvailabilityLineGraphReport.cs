...
{
	public class AvailabilityLineGraphReport
	{
		SummaryReportData data;

		public AvailabilityLineGraphReport (SummaryReportData data)
		{
			this.data = data;
		}
		...
		public string BuildReport ()
		{
			...
			BasicXmlDocument.AppendAttribute(yAxis, "title", "Availability (%)");
			...
			BasicXmlDocument.AppendAttribute(line, "colour", CONVERT.ToComponentStr(CoreUtils.SkinningDefs.TheInstance.GetColorData("pdf_report_availability_graph_colour", Color.DarkGreen)));
			...
			for (int round = 1; round <= data.RoundsPlayed; round++)
			{
				...
				BasicXmlDocument.AppendAttribute(point, "y", data.Availability[round - 1]);
				BasicXmlDocument.AppendAttribute(point, "dot", true);
			}

			string filename = data.GameFile.Dir + @"\global\AvailabilityReport.xml";
			xml.Save(filename);
			return filename;
		}
	}
