...
{
	/// <summary>
	/// This is a light modified version of the DC opsControl Panel 
	/// Just removed the facilities items and kept the extra popup panels rather than a full refactor
	/// </summary>
	public class OpsControlPanel_AOSE : OpsControlPanel
	{
		protected bool Allow_AWT_In_Round1 = false;

		protected ImageButton optionsButton;
		protected ImageButton critAdvButton;
		...
		public OpsControlPanel_AOSE (IOpsScreen screen, NodeTree nt, IncidentApplier iApplier, MirrorApplier mirrorApplier, 
				...
				IsTrainingFlag, OperationsBackColor,  GroupPanelBackColor, gameFile)
		{
			...
			optionsButton.Location = new Point(upgradeButton.Right + buttonseperation, 0);
			Controls.Add(optionsButton);

			//CriticalAdvanatge Button
			critAdvButton = ImageButton.FromAutoVariants(0, "/images/buttons/critadv.png");
			critAdvButton.Click += CritAdvButton_Click;
			critAdvButton.Size = new Size(buttonwidth, buttonheight);
			//optionsButton.Location = new Point (unplugButton.Right + buttonseperation, 0);
			critAdvButton.Location = new Point(optionsButton.Right + buttonseperation, 0);
			Controls.Add(critAdvButton);
			...
			pendingActions.Location = new Point(critAdvButton.Right + buttonseperation, 0);
		...
		/// <summary>
		/// This class needs to stripped back and rebuilt but we can just move the buttons for now
		/// 
		/// </summary>
		public void MoveButtons()
		{
            ...
            critAdvButton.Location = new Point (installBusinessServiceButton.Right + 10, 0);
            critAdvButton.Size = new Size (buttonwidth, buttonheight);

            pendingActions.Location = new Point (critAdvButton.Right + 10, 0);
            pendingActions.Size = new Size (buttonwidth, buttonheight);
		}
		...
		public override void ResetButtons ()
		{
			...
			if (critAdvButton != null)
			{
				critAdvButton.Active = false;
			}
		}
		...
		protected override void installBusinessServiceButton_ButtonPressed (object sender, ImageButtonEventArgs args)
		{
			if (playing)
			{
				InstallBusinessServiceControl_AOSE IBS_Panel = new InstallBusinessServiceControl_AOSE(this, _network, 
					round, _prjmanager, MyIsTrainingFlag, MyOperationsBackColor, MyGroupPanelBackColor, gameFile);
				IBS_Panel.Set_MTRS_Visibility(false);
		...
		{
			AOSE_ZoneOptionsPanel panel = new AOSE_ZoneOptionsPanel(this, _network, MyIsTrainingFlag, MyOperationsBackColor, MyGroupPanelBackColor, round);
			panel.Size = new Size(popup_width, popup_height);
			panel.Location = new Point(popup_xposition, popup_yposition);
			Parent.Controls.Add(panel);
			panel.BringToFront();
		...
		void CritAdvButton_Click(object sender, EventArgs e)
		{
			AOSE_CritAdvOptionsPanel panel = new AOSE_CritAdvOptionsPanel(this, _network, MyIsTrainingFlag, MyOperationsBackColor, MyGroupPanelBackColor, round);
			panel.Size = new Size(popup_width, popup_height);
			panel.Location = new Point(popup_xposition, popup_yposition);
			Parent.Controls.Add(panel);
			panel.BringToFront();
		...
		{
			UpgradeAppControl_DC upgradePanel = new UpgradeAppControl_DC (this, _iApplier, _network, 
				true, MyIsTrainingFlag,  MyOperationsBackColor, MyGroupPanelBackColor, playing);
		...
		protected virtual void GenerateOperationPanel_MTRSOrAutoRestore ()
		{
			...
			//                                                        new EventHandler [] { new EventHandler (SetMTRS_Click),
			//                                                                              new EventHandler (SetAutoRestore_Click) } );

			IntermediateOpsPanel panel = new IntermediateOpsPanel(this, MyIsTrainingFlag, MyOperationsBackColor, 200,
																														 "Set Auto Restore Times",
																															new string[] {"Set Auto Restore" },
																															new EventHandler[] {SetAutoRestore_Click });
