...
{
	/// <summary>
	/// Like a BouncyNodeMonitor, but using the VariableHeightMonitorBar as a basis.
	/// </summary>
	public class VariableHeightBouncyNodeMonitor : VariableHeightMonitorBar, ITimedClass
	{
		...
		public VariableHeightBouncyNodeMonitor(Node _monitoredEntity, Random r)
			: base(new int [] {2, 2, 2, 6, 4} )
		{
			monitoredEntity = _monitoredEntity;
			random = r;

			SetLevelColors(new Color [] {Color.Green, Color.Green, Color.Green, Color.Orange, Color.Red});

			// Pick a random time until the next big jump...
			ticksToBigJump = random.Next(8);

			if(null != monitoredEntity && !monitoredEntity.GetBooleanAttribute("is_saas", false))
			{
		...
		{
			StopMonitoringEntity();
		}

		void StopMonitoringEntity()
		{
			if (null != timer)
			{
				if (null != watcher)
		...
		void monitoredEntity_AttributesChanged(Node sender, ArrayList attrs)
		{
			foreach(AttributeValuePair avp in attrs)
			{
				...
				if (avp.Attribute == "is_saas" && avp.Value.ToLower() == "true")
				{
					// stop the monitor!!
					StopMonitoringEntity();
				}
