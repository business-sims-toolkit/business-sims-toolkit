...
{
	public class CommonGameDetails : HPGameDetails
	{
		class custom_info
		{
		...
		public CommonGameDetails (GameManagement.NetworkProgressionGameFile gameFile, IGameLoader gamePanel)
			: base(gameFile)
		{
		...
		public new virtual void BuildNewControls ()
		{
			...
			btn_LoadCustom = SkinningDefs.TheInstance.CreateWindowsButton();
		...
		protected virtual void ConfigureNewControls (bool recurse)
		{
			...
			switch (gameEvalType)
			{
                ...
                case em_GameEvalType.ESM:
                    gameType.SelectedIndex = gameType.Items.IndexOf("ESM");
                    break;
		...
		protected void gameType_SelectedIndexChanged (object sender, EventArgs e)
		{
			if (ignoreTypeSelect == false)
			{
				switch (gameType.SelectedItem.ToString().ToUpper())
				{
					...
					case "LEAN":
						gameCustomList.Enabled = false;
						gameEvalType = em_GameEvalType.ESM;
						break;
