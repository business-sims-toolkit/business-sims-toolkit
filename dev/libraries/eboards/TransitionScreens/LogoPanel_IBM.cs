...
namespace TransitionScreens
{
	...
	/// </summary>
	public class LogoPanel_IBM : LogoPanelBase
	{
		/// <summary> 
		/// Required designer variable.
		/// </summary>
		System.ComponentModel.Container components = null;
		protected PictureBox FacLogoPicBox = null;
		...
		public LogoPanel_IBM()
		{
			...
			TeamLogoPicBox.Location = new Point(20+12,14);
		...
		{
			SetWideMode(wide, TrainingGameFlag);
			if (TrainingGameFlag)
			{
		...
		public void SetWideMode (bool wide, bool training)
		{
			this.wide = wide;

			if (wide)
			{
				FacLogoPicBox.Location = new Point(400-12,14);

				if (training)
				{
					BackgroundImage = Repository.TheInstance.GetImage(AppInfo.TheInstance.Location + 
						"\\images\\panels\\t_bg_logos_wide.png");
				}
				else
				{
					BackgroundImage = Repository.TheInstance.GetImage(AppInfo.TheInstance.Location + 
						"\\images\\panels\\bg_logos_wide.png");
				}
			}
			else
			{
				FacLogoPicBox.Location = new Point(256,14);
				if (training)
				{
					BackgroundImage = Repository.TheInstance.GetImage(AppInfo.TheInstance.Location + 
		...
		public void ShiftLogos (Rectangle left, Rectangle right)
		{
			TeamLogoPicBox.Location = new Point (left.Left + ((left.Width - TeamLogoPicBox.Width) / 2), left.Top + ((left.Height - TeamLogoPicBox.Height) / 2));
			FacLogoPicBox.Location = new Point (right.Left + ((right.Width - FacLogoPicBox.Width) / 2), right.Top + ((right.Height - FacLogoPicBox.Height) / 2));
		}
	}
}