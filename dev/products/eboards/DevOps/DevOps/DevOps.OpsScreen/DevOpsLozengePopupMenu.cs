...
{
	public class DevOpsLozengePopupMenu : Form
	{
		...
		public static void AddMenuItem (DevOpsLozengePopupMenu menu, string title, EventHandler action,
		                                bool enabled, string imageFileName = "")
		{
		...
		public class DevOpsLozengePopupMenuItem : BasePanel
		{
			...
			string GetDisabledImageName (string image)
			{
				...
				{
					return System.IO.Path.Combine(System.IO.Path.GetDirectoryName(image), System.IO.Path.GetFileNameWithoutExtension(image) + "_disabled" + System.IO.Path.GetExtension(image));
				}
			}
			...
			public DevOpsLozengePopupMenuItem (string text, float fontSize, FontStyle style)
				: this (text, null, null, fontSize, style)
			{
			}

			public DevOpsLozengePopupMenuItem (string text, string imageFilename, float fontSize, FontStyle style)
				: this (text, imageFilename, null, fontSize, style)
			{
			}

			public DevOpsLozengePopupMenuItem (string text, string imageFilename, string checkedImageFilename, float fontSize, FontStyle style)
			{
				margin = 8;
				imageSize = 16;
            ...
            DevOpsLozengePopupMenu Menu
            {
                get
                {
                    var parent = Parent;
                    while ((parent != null)
                        && !(parent is DevOpsLozengePopupMenu))
                    {
                        parent = parent.Parent;
                    }

                    return (DevOpsLozengePopupMenu)parent;
                }
            }
			...
			void label_MouseEnter (object sender, EventArgs e)
			{
				if (Clickable)
				{
					BackColor = Lerp(0.2f, backColour, Color.Black);
				}
			}
		...
		readonly List<DevOpsLozengePopupMenuItem> items;
		...
		public DevOpsLozengePopupMenu ()
		{
			items = new List<DevOpsLozengePopupMenuItem> ();
		...
		public DevOpsLozengePopupMenuItem AddHeading (string text)
		{
			var item = new DevOpsLozengePopupMenuItem (text, null, 11, FontStyle.Bold);
			items.Add(item);
		...
		public DevOpsLozengePopupMenuItem AddHeading (string text, string imageFilename)
		{
			var divider = AddDivider(6, false);
			Controls.Remove(divider);
			divider.SetColours(Color.Transparent, Color.Black);

			var item = new DevOpsLozengePopupMenuItem (text, imageFilename, 10, FontStyle.Bold);
			items.Add(item);
			item.Clickable = false;
			item.SetColours(Color.Transparent, Color.Black);
			item.CornerRounding = cornerRounding;
		...
		public DevOpsLozengePopupMenuItem AddItem (string text, string imageFilename = "", string checkedImageFilename = "")
		{
			var item = new DevOpsLozengePopupMenuItem (text, imageFilename, checkedImageFilename, 9, FontStyle.Regular);
			items.Add(item);
			item.Clickable = true;
			item.CornerRounding = cornerRounding;
			Controls.Add(item);
		...
		public DevOpsLozengePopupMenuItem AddDivider ()
		{
			return AddDivider(8, true);
		}

		public DevOpsLozengePopupMenuItem AddDivider (int height, bool drawLine)
		{
			var item = new DevOpsLozengePopupMenuItem("", null, 10, FontStyle.Regular)
			{
				IsDivider = true,
		...
		public IList<DevOpsLozengePopupMenuItem> Items => new List<DevOpsLozengePopupMenuItem> (items);

		public void Show (Control parentContainer, Control showingControl, Point screenPosition)
        {
            ...
            if (showingControl != null)
            {
                ...
                if ((new Rectangle (menuLocation, Size)).IntersectsWith(new Rectangle(ourTopLeft, showingControl.Size)))
                {
