  ...
  //   if all 8 ( 4 stores * instore/online) are up then it's 0%
	...
	public class SystemImpactMonitor
	{
		...
		private void storeNode_ChildAdded(Node sender, Node child)
		{
			...
			if (TheStoreBSUNodes.ContainsKey(name)==false)
			{
		...
		private void storeNode_ChildRemoved(Node sender, Node child)
		{
			...
			string name = child.GetAttribute("name");
			if (TheStoreBSUNodes.ContainsKey(name))
			{
			...
			if (attrs != null)
			{
				if (attrs.Count > 0)
				{
					foreach(AttributeValuePair avp in attrs)
					{
						...
						else if ((attribute.ToLower()=="transaction_type"))
						{
							RebuildImpactValue(); 
						}
						else if ((attribute.ToLower()=="has_impact"))
						{
							RebuildImpactValue();
						}
					}
