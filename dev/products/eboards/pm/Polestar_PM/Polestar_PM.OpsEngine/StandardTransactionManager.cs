...
{
	/// <summary>
	/// The Standard V3 Transaction Manager 
	/// 
	...
	/// </summary>
	public class StandardTransactionManager : BaseClass, ITimedClass
	{
		...
		private bool UseDirectRevenueFromTransaction = true;
		
		public StandardTransactionManager(NodeTree nt, int round, bool kill_previous)
		{
		...
		public void Dispose()
		{
			...
			this.MyNodeTree = null;
			this.RevenueNode = null;
			
			if (TransactionValuesNode != null)
			{
				TransactionValuesNode = null;
			}
		}
		...
		private int DetermineBatchRevenue(string destination_store, string eventtype, int rev_level)
		{
			...
			if (StoreNode != null)
			{
				...
				if (TransactionAmounts.ContainsKey(identifier))
				{
				...
				foreach(AttributeValuePair avp in attrs)
				{
					if(avp.Attribute == "seconds")
					{
						...
						foreach(Node n in TransactionsNode.getChildren())
						{
							...
							if ((status.ToLower() == "queued")||(status.ToLower() == "delayed"))
							{
								...
								if (node_time == global_time)
								{
									...
									if (UseDirectRevenueFromTransaction)
									{
										int direct_rev_amount = n.GetIntAttribute("revvalue",0); 	
										batch_value = direct_rev_amount;
									}
									else
									{
									...
									{
										n.SetAttribute("status","Cancelled");
						...
						foreach(Node n in TransactionsNode.getChildren())
						{
							...
							if (destination_store.ToLower() == storename.ToLower())
							{
								if (eventtype == "instore")
								{
									...
									if ((InStorePortalWorking)&&(status.ToLower() == "delayed"))
									{
					...
					if(avp.Attribute == "up_online")
					{
						...
						foreach(Node n in TransactionsNode.getChildren())
						{
							...
							if (destination_store.ToLower() == storename.ToLower())
							{
								...
								{
									//portal good and transaction showing "Delayed" --> back to "Queued"	
									if ((OnLinePortalWorking)&&(status.ToLower() == "delayed"))
									{
										n.SetAttribute("status","Queued");
										//System.Diagnostics.Debug.WriteLine("  Changed");
									}
		...
		private void TransactionsNode_ChildAdded(Node sender, Node child)
		{
			...
			if (child != null)
			{
				...
				{
					child.SetAttribute("status", "Delayed");
				}
			}
